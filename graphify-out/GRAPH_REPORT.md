# Graph Report - C:\Users\sgarm\Downloads\ledger_web  (2026-09-18)

## Corpus Check
- cluster-only mode — file stats not available

## Summary
- 692 nodes · 531 edges · 270 communities (245 shown, 25 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 31 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `06404e0d`
...
5. `📸 Screenshots` - 9 edges
6. `get_sold_qty_col_idx()` - 8 edges
7. `⚡ Ledger God Mode Web App` - 8 edges
8. `update_inventory_formulas()` - 7 edges
9. `restore_from_gsheets()` - 7 edges
10. `5. Sequence Diagrams & Data Flows` - 7 edges
...
Cohesion: 0.07
Nodes (28): add_product(), api_force_sync(), api_sync_now(), _auto_sync_loop(), Manual trigger: pull from GSheets → check rollover → push back., init_google_sheets(), cancel_invoice(), create_invoice() (+20 more)

### Community 1 - "Community 1"
Cohesion: 0.07
Nodes (27): 🧮 1. Dynamic Monthly Inventory Engine (`inventory_engine.py`), 🧾 2. Invoice Billing Terminal (`invoice_api.py` Blueprint), 🔄 3. Monthly Auto-Rollover + Hourly GSheets Sync, 🤖 4. Portal Robotics (Playwright 1.44), 🌲 5. Genealogy Downline Crawler (`Full_Tree_Crawler.py`), 🩺 6. Disease Guide, 🤖 AWPL Portal — Bill Detail, 🤖 AWPL Portal — Login (+19 more)
...
Cohesion: 0.13
Nodes (9): add_product(), api_inventory_master(), api_inventory_master_months(), api_ledger_report(), api_mizoram_bronze(), api_mizoram_bronze_update(), api_product_sales(), get_db() (+1 more)
...
Cohesion: 0.20
Nodes (6): api_inventory(), api_kpi(), categorize_product(), compute_kpis_data(), dashboard(), Categorize an Asclepius product into authentic product lines.
...
Cohesion: 0.31
Nodes (9): api_inventory_add_product(), api_inventory_master_update(), api_inventory_restock(), api_update(), index(), Recalculate formulas for a row.      BUG FIX: The old implementation summed on, Recalculate the TOTAL row at the bottom., update_inventory_formulas() (+1 more)
...
## Knowledge Gaps
- **95 isolated node(s):** `cookies`, `origins`, `inventory`, `currentSort`, `filterInput` (+90 more)
...
- **Why does `get_db()` connect `Community 4` to `Community 0`, `Community 2`, `Community 36`, `Community 10`, `Community 12`, `Community 13`?**
