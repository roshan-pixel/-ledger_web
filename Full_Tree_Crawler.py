...
START_NODE = "62C04A"  # We start from ROOT to ensure we get BOTH SAO and SGO completely
...
async def crawl_tree(ds_code, password):
    ...
    if os.path.exists(BACKUP_FILE):
        ...
        try:
            ...
            # Important: If resuming, queue must be repopulated with nodes that haven't been visited but are in the tree.
            # To be perfectly safe, we'll just start from ROOT and skip visited nodes quickly without fetching them again
            # Actually, to make resume work, we just start queue with ROOT and let it BFS fast-forward.
    ...
    async with async_playwright() as p:
        ...
        print(f"3. Beginning BFS Extraction from Root node: {START_NODE}")
        ...
        while queue:
            ...
            print(f"[{len(visited)}] Extracting Node: {current_node} | Queue size: {len(queue)}")
            ...
            try:
                ...
                if await search_input.count() > 0:
                    ...
                    await page.wait_for_load_state("networkidle")
                    await page.wait_for_timeout(1500) # Increased timeout slightly to be safe
                else:
                ...
                if not root_a or root_a.text.strip() != current_node:
                    print(f"  -> Skipping {current_node}, not found on page.")
                    continue
                ...
                # Update or Add
                results = [r for r in results if r['DS Code'] != current_node] # Remove old if exists
                results.append({
